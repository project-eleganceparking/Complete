...
public class WoowaDAO {
	...
	public int addUser(UserVO user) {
		if (user.getEmail_dns().length() > 1) {
			user.setUseremail(user.getUseremail() + "@" + user.getEmail_dns());
		} else {
			user.setUseremail(user.getUseremail() + "@" + user.getDns_Add());
		}
	...
	public int deleteComment(CommentVO comment){ //댓글 삭제
		return sqlSession.delete("lab.mybatis.mapper.CommentMapper.deleteComment", comment);
	}
