...
<head>    
 ...
 <script src="./source/js/mydelete.js"></script>
    ...
    <div id="primary" class="content-area column full">
      <main id="main" class="site-main">
		...
		<div class="hButton1">	
			...
			<input type="button" class="headerButton" id="MyPosition"   value="내 위치">
		</div>	
		</c:if>
		...
		<div class="hButton2">
			<div class="dropb" >
				<button class="headerButton" style="color:white"><a>즐겨 찾기</a></button> 
                   ...
                   <div>
                   <div>주차위치</div>
                   <c:forEach var="myPark" items="${Mypark}">                   	
                    <a href="#"><a class="dropdownsub">${myPark.usermail}</a></a>
                    </c:forEach>
                    
                   <div>즐겨찾기</div>
                   <c:forEach var="myFav" items="${MyFav}">
                    <a href="#"><a class="dropdownsub">${myFav.favlat}</a></a>
 ...
 <!-- Main Content 끝 --> 
  ...
  <tr><td colspan=2 height=1 background=./source/dotline.gif></td></tr>  
    <tr><td width=100>
      ...
      <button id='c${comment.cid}' 
      onClick='mydelete(${comment.cid})'>x</button>  
      </span>
     
      <!-- 댓글 삭제 폼 -->
      <span id='${comment.cid}' style='display:none'>
        <form method='post' action=./delete_comment.do>   
          <input type= 'hidden'  name='page' value='${page}'>
          암호 <input  type=password name='password' size=10 placeholder="" value='${comment.password}'>
          <input type=hidden name='cid' value='${comment.cid}'>
          <input type=hidden name='writer' value='${comment.writer}'>
          <input type=hidden name='markerid' value=''> <%-- ${lots.prkplceNm} --%>
  ...
  <!-- 댓글쓰기 -->
  <form id='comment' method='post' action='./comment.do'>
  <input type='hidden' name='markerid' value=''> <!-- 선택한 주차장마커번호로 id저장 -->
  <input type= 'hidden'  name='page' value='${page}'>
  ...
  <table width='95%'>
    ...
    <tr><td align=right>
        이름<input type='text' name='writer' size='10' value='${user.username}'> <!-- username이 writer로 -->
        비밀번호<input type='password' name='password' value='${comment.password}' size='10' placeholder="">
